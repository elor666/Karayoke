    ...
    def connect_server(self, root: ctk.CTk, ip_entry, port_entry):
        ...
        if port.isdigit() and all(ch in "1234567890." for ch in ip):
            connection_thread = threading.Thread(
                target=self.try_connection, args=(ip, int(port)))
...
class SearchPage(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.search_button = ctk.CTkButton(
            ...
            height=56,
            hover_color="#C85C8E",
            command=lambda: self.get_search(master),
            corner_radius=200,
            text_color="#FFBABA")
        self.search_button.place(anchor='e', relx=0.445, rely=0.4)

        self.songs_menu = ctk.CTkOptionMenu(
            self,
            values=[],
        ...
        self.vocals_check = ctk.CTkCheckBox(
            self,
            text="No Vocals",
            variable=self.no_vocals,
            onvalue=True,
            offvalue=False,
            checkmark_color="#FFBABA",
            fg_color="#7B2869",
            font=(
                "Roboto",
                21),
        ...
        self.play_button = ctk.CTkButton(
            ...
            text="Play Song",
            fg_color="#7B2869",
            command=lambda: self.get_song_details(master),
            width=280,
            height=56,
            hover_color="#9D3C72",
            text_color="#FFBABA")
        ...
        self.exit_kar = ctk.CTkButton(
            self,
            text="Exit",
            command=self.exit_karyoke,
            fg_color="#7B2869",
            hover_color="#9D3C72")
    ...
    def get_search(self, root):
        ...
        if all(
                ch not in "#" for ch in artist) and all(
                ch not in "#" for ch in song):
    ...
    def get_song(self):
        ...
        msg_to_send = str(self.values.index(self.choice)) + \
            "#" + str(int(self.auto_sync.get()))
...
class KarayokePage(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.text_lines = [equlize_words(line[:-1]) if line[-1] == '\n' else equlize_words(
            line) for line in self.text_lines if line != "\n"]

        self.times = TIMES

        self.scroll_index = 0

        self.labels = [ctk.CTkLabel(self, text_color="white", text=line, font=(
            'Roboto', 35)) for line in self.text_lines[:LINE_COUNT]]
        ...
        self.time_slider = ctk.CTkSlider(
            ...
            button_color="#7B2869",
            button_hover_color="#9D3C72")  # , number_of_steps=round(audio_length//0.1)
        ...
        self.time_label = ctk.CTkLabel(
            self, text_color="white", text="0", font=(
                'Roboto', 25))
        ...
        if not pygame.mixer.music.get_busy():
            if event.delta > 0:
                ...
                self.scroll_index = min(
                    len(self.times) - LINE_COUNT, self.scroll_index + 1)

            self.update_labels(index=self.scroll_index)

    def get_time_index(self, cur_time):
        """Return the index of the time in the times list"""
    ...
    def update_labels(self, index=None):
        ...
        if value > floor(
                LINE_COUNT / 2) and value <= len(self.text_lines) - ceil(LINE_COUNT / 2):
            for i in range(LINE_COUNT):
                self.labels[i].configure(text=self.text_lines[value - floor(
                    LINE_COUNT / 2) + i], text_color="white", font=('Roboto', 35))
            ...
            for i in range(LINE_COUNT):
                self.labels[i].configure(text=self.text_lines[len(
                    self.text_lines) - LINE_COUNT + i], text_color="white", font=('Roboto', 35))

        if index is None:
            self.bold_line(value)
            if value < len(self.text_lines) - 1:
                if pygame.mixer.music.get_busy():
                    self.boldid = self.after(
                        (int((self.get_time_range(value + 1, cur_time))) * 1000), self.update_labels)
    ...
    def play_music(self):
        if not pygame.mixer.music.get_busy():
            ...
            if index == 0:
                # int(self.get_time_range(self.get_time_index(cur_time),cur_time)*1000)
                self.boldid = self.after(int(self.get_time_range(
                    index, cur_time) * 1000), self.update_labels)
            else:
    ...
    def pause_music(self):
        if pygame.mixer.music.get_busy():
            ...
            self.time_slider.set(
                self.start + pygame.mixer.music.get_pos() / 1000)
    ...
    def update_start(self, value):
        if not pygame.mixer.music.get_busy():
            ...
            if not self.time_slider.get() < self.times[0]:
                ...
                [label.configure(text_color="white", font=('Roboto', 35))
    ...
    def bold_line(self, index):
        ...
        if index < len(self.text_lines):
            ...
            [label.configure(text_color="#FFBABA", font=('Roboto', 35)) if i < index else label.configure(
                text_color="white", font=('Roboto', 35)) for i, label in enumerate(self.labels)]
            self.labels[index].configure(
                text_color="#C85C8E", font=(
                    'Roboto', 37))
...
class KarayokePageNO(ctk.CTkFrame):
    ...
    def mouse_scroll(self, event):
        if event.delta > 0:
            ...
            self.scroll_index = min(
                len(self.text_lines), self.scroll_index + 1)
